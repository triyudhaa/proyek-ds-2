...
# -------------------------
# 8) Visualization params and create thumbnail URL
# -------------------------
classification_vis = {
    'min': 0,
    'max': 1,
...
# -------------------------
# 9) Download thumbnail and display locally
# -------------------------
...
# plt.imshow(img)
# plt.axis('off')
# plt.title('GEE Prediction (classified)')
# plt.show()

# optionally save to disk
out_path = "Training.png"
img.save(out_path)
print(f"Saved prediction image to {out_path}")

# -------------------------
# 10) Function Coastline
# -------------------------

def sliding_window_majority(arr, window_size=7):
    """
    Fungsi untuk mengubah nilai array biner (0/1) berdasarkan mayoritas tetangga.
    ...
    window_size : int
        Ukuran jendela (harus ganjil), default 3x3.
...
def clean_mask(mask, target_value, min_size):
  """
  Bersihkan komponen kecil dari kelas tertentu.
  target_value: 1 (air) atau 0 (darat).
  """
...
def read_geotiff(filepath):
    """
    Baca GeoTIFF dan kembalikan array beserta metadata
...
def extract_coastline_from_geotiff(filepath, water_value=1, land_value=0, ws = 7):
    """
    Ekstrak garis pantai dari GeoTIFF

    Parameters:
    -----------
    filepath : str
        Path ke file GeoTIFF (0=darat, 1=air)
    water_value : int/float
        Nilai yang merepresentasikan air dalam GeoTIFF
    land_value : int/float
        Nilai yang merepresentasikan darat dalam GeoTIFF

    Returns:
    --------
    ocean_mask : numpy.ndarray
        Mask dari laut (tidak termasuk danau)
    contours : list
    ...
    array = clean_mask(array, target_value=1, min_size=1000)
    array = clean_mask(array, target_value=0, min_size=500)
    array = sliding_window_majority(array, window_size=7)
...
newImageCropped = newImage.clip(area)
kernel = ee.Kernel.gaussian(radius=4, sigma=0.7, units='pixels')
newImageCropped = newImageCropped.convolve(kernel)
newImageCropped = newImageCropped.select(bands)
newClassification = newImageCropped.classify(classifier)

# Visualize via .visualize() and request a thumbnail
viz_image = newClassification.visualize(min=classification_vis['min'],
                                 max=classification_vis['max'],
                                 palette=classification_vis['palette'])
...
filename = f"prediction_final_{year}_{period}.tif"
with open(filename, "wb") as f:
    f.write(response.content)
# tiff_bytes = BytesIO(response.content)

# Baca GeoTIFF langsung dari memori (tanpa file)    
with rasterio.open(filename) as src:
    raw = src.read(1)
    transform = src.transform
# print(raw)

# 2. Binerisasi (air=1, darat=0)
correction = np.where(raw == 58, 1, 0).astype(np.uint8)

# 3. Connected component labeling
labeled, num_features = ndimage.label(correction)

# 4. Hapus patch kecil (flood fill)
correction = clean_mask(correction, target_value=1, min_size=1000)  # air
correction = clean_mask(correction, target_value=0, min_size=500)  # darat
correction = sliding_window_majority(correction, window_size=7)
print(correction)
...
plt.title(f"Hasil Prediksi Sentinel2 {period.replace('_', '-')} {year}\n(Sebelum Koreksi)")
# Sesudah koreksi
plt.subplot(1, 2, 2)
plt.imshow(correction, cmap="coolwarm")
plt.colorbar(label="Kelas Prediksi")
plt.title(f"Hasil Prediksi Sentinel2 {period.replace('_', '-')} {year}\n(Sesudah Koreksi Flood Fill)")

plt.tight_layout()
plt.show()

# --------
# 12) Pembuatan Garis Pantai 
# --------
result_coastline = []

ocean_mask, contours_pixel, contours_geo, meta, array = extract_coastline_from_geotiff(
    filename,
    water_value=1,
    land_value=0,
    ws = 7
...
# --- plot dengan koordinat geo (lon-lat) ---
plt.figure(figsize=(10, 8))

for contour in contours_geo:
    xs = [pt[0] for pt in contour]  # longitude
    ys = [pt[1] for pt in contour]  # latitude
    plt.plot(xs, ys, linewidth=2, label=f"{period.upper()} {year}")

plt.title(f"Coastline {period.upper()} {year}")
